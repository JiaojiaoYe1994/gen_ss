...
# todo
from plot_mask import plot_seg_result

from tensorboardX import SummaryWriter
from model import HDMNet

from util import dataset
from util import dataset_aug_demo as dataset
from util import transform, transform_tri, config
from util.util_demo import AverageMeter, poly_learning_rate, intersectionAndUnionGPU, get_model_para_number, setup_seed, get_logger, get_save_path, \
...
# ----------------------  DATASET  ----------------------
    ...
    if args.evaluate:
        ...
        if args.data_set == 'pascal' or args.data_set == 'coco':
            print("**shot=args.shot, dat    a_root=args.data_root, base_data_root=args.base_data_root, ** " , args.shot, args.data_root,args.base_data_root,)
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b, image_path, label_path, support_image_path, support_label_path) in enumerate(val_loader):
            ...
            support_images = [np.array(Image.open(support_image_path[0]))]
#             support_masks = [np.array(Image.open(support_label_path[0]))]
            query_image = np.array(Image.open(image_path[0]))
#             ground_truth_mask = np.array(Image.open(label_path[0])) 
            
#             print(support_image_path[0], support_label_path[0], image_path[0], label_path[0])

#             predicted_mask = np.array(Image.open('/mnt/nas/jiaojiao/2024/HDMNet/dm/query_label/{}.png'.format(query)))

#             # input_image = np.array(Image.open('/mnt/nas/jiaojiao/2024/HDMNet/{}.jpg'.format(query)))
#             # output_mask = np.array(Image.open('/mnt/nas/jiaojiao/2024/HDMNet/output_mask.png'))

            support_masks = [cv2.resize(np.array(torch.squeeze(s_mask).cpu().detach().numpy()), (support_images[0].shape[1], support_images[0].shape[0]), interpolation=cv2.INTER_NEAREST)] # label 
            size1 = query_image.shape
#             size2 = output_mask.shape

            output_cp = torch.squeeze(output.max(1)[1])
            target_cp = torch.squeeze(target)
      
            transform = transforms.ToPILImage()
            output_mask = transform(output_cp.to(torch.uint8))
#             output_mask = output_cp.cpu().detach().numpy()
            ground_truth_mask = target_cp.cpu().detach().numpy()

            img2_resized = cv2.resize(np.array(output_mask), (size1[1], size1[0]), interpolation=cv2.INTER_NEAREST) # label rescale
            ground_truth_mask = cv2.resize(np.array(ground_truth_mask), (size1[1], size1[0]), interpolation=cv2.INTER_NEAREST) # label rescale
            print("predict shape: ", img2_resized.shape,  query_image.shape,ground_truth_mask.shape, output.shape, target.shape)
        
            print("predict class label:", np.unique(img2_resized), img2_resized.min(), img2_resized.max())
            from plot_mask import visualize_few_shot_segmentation
            visualize_few_shot_segmentation(support_images, support_masks, query_image, img2_resized, ground_truth_mask[:,:] , image_path[0].split("/")[-1])
...
#             predict = transform(output.cpu().detach().numpy())
#             predict = predict.astype(np.uint8) 
#             print("label_path: ", label_path)
#             image.save('input_image.png')
#             predict.save('output_mask.png',mode="L")
#             predict.save("/mnt/nas/jiaojiao/2024/HDMNet/dm/predict/" + label_path[0].split("/")[-1])  
#             print(input.shape, output.shape, subcls)
#             print(input.min(), input.max(), output.min(), output.max())
#             print(input.dtype(), output.dtype())
#             img_pre = plot_seg_result(input.cpu().detach().numpy(), output.cpu().detach().numpy(),type="red", size=None)
